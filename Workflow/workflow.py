...
class Workflow:
    """
    Encapsulates the workflow logic using LangGraph.

    Attributes:
        graph_builder: The StateGraph builder instance.
        memory: The MemorySaver instance for state checkpoints.
        graph: The compiled StateGraph.
    """

    def __init__(self):
        """
        Initializes the workflow by building and compiling the StateGraph.
        """
    ...
    def get_response(self, question: str) -> str:
        """
        Executes the workflow and retrieves the response for a given question.

        Args:
            question: The input question provided by the user.

        Returns:
            The result string if available; otherwise, an empty string.
        """
        ...
        try:
            ...
            # Retrieve the last message from the events
            last_message = None
            for event in events:
